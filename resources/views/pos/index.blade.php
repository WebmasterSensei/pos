    ...
    <div class="pos-left">
        ...
        <div class="category-pills" id="categoryPills">
            ...
            @foreach($categories as $cat)
                <button class="pill" data-category="{{ $cat }}" onclick="filterCategory(this, '{{ $cat }}')">{{ $cat }}</button>
            @endforeach
        </div>
...
<div class="modal-overlay" id="receiptModal">
    <div class="modal">
        <div style="text-align:center;margin-bottom:16px;">
            ...
            <div style="font-family:var(--font-mono);font-size:10px;color:var(--text-muted);letter-spacing:0.1em">NEXUS POS · RECEIPT</div>
        </div>
        ...
        <hr class="receipt-divider">
        <div style="text-align:center;font-family:var(--font-mono);font-size:9px;color:var(--text-muted);letter-spacing:0.06em;margin-bottom:16px;">
...
<div class="modal-overlay" id="discountModal">
    <div class="modal">
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" id="discountAmt" placeholder="0" min="0" max="100">
        </div>
        <div class="modal-actions">
...
// ===== STATE =====
let cart = [];
let discount = { type: 'percent', val: 0 };
const TAX_RATE = 0.12;
let scanBuffer = '';
let scanTimer = null;
...
function processBarcode(code) {
    ...
    const card = document.querySelector(`[data-barcode="${code}"]`);
    if (card && card.dataset.stock > 0) {
        addToCart(card);
        showToast(`✓ Scanned: ${card.dataset.name}`, 'success');
    } else if (card) {
        showToast(`✕ ${card.dataset.name} is out of stock`, 'error');
    } else {
...
function addToCart(card) {
    ...
    setTimeout(() => card.classList.remove('scan-flash'), 400);
}
...
function updateDiscountLabel() {
    ...
    document.getElementById('discountAmtLabel').textContent = type === 'percent' ? 'Discount (%)' : 'Discount Amount (₱)';
}
...
function showToast(msg, type = 'success') {
    ...
    setTimeout(() => { toast.style.opacity = '0'; toast.style.transform = 'translateX(100%)'; toast.style.transition = '0.3s'; setTimeout(() => toast.remove(), 300); }, 2500);
}
